...
function Table({ main }) {
  ...
  const maincategoryData = useSelector(
    (state) => state.mainCategoryState.mainCategory
  );
  ...
  console.log("Table Data SubCategory all ", all);
  axios
  ...
  const columns = [
    ...
    },
    {
      ...
      renderCell: (params) => {
        ...
        const services = all.mainCategory.Services.filter(
          (item) => item.name === categoryName
        );
        const imageSrc = services.length > 0 ? services[0].image : "";
